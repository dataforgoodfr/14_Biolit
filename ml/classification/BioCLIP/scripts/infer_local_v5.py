"""
INFÉRENCE LOCALE — Hybride V4 Whitening + PCA
==============================================
Classifie des images locales avec le système hybride v4.
...
def infer_images(image_paths, models, device, batch_size=32):
    ...
    for j50, sp in idx_to_sp50.items():
        for j100, sp100 in idx_to_sp100.items():
            if sp == sp100:
                idx50_to_idx100[j50] = j100
                break
    ...
    for i, path in enumerate(tqdm(valid_paths, desc="Classification")):
        ...
        if methode in ("classifier_top50", "classifier_top50_low"):
            # Projeter probs50 → espace 100 espèces (rares à 0)
            p_tax = np.zeros(len(idx_to_sp100))
            for j50, j100 in idx50_to_idx100.items():
                p_tax[j100] = probs50_np[i, j50]
            # p_tax somme déjà à ~1 (softmax sur 50 communes uniquement)

        elif methode == "prototypical_rare":
            # Utiliser directement les probas prototypes sur 100 espèces
            p_tax = probs100_np[i]

        else:  # rejected
            # Fusion équilibrée : on n'a pas de décision ferme, on combine
            p_tax = np.zeros(len(idx_to_sp100))
            for j50, j100 in idx50_to_idx100.items():
                p_tax[j100] = 0.5 * probs50_np[i, j50] + 0.5 * probs100_np[i, j100]
            # Rares (non couverts par classifier)
            for j100 in range(len(idx_to_sp100)):
                if j100 not in idx50_to_idx100.values():
